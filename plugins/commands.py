...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, UPDATES, SUPPORT, MYSUP
...
@Client.on_message(
    (
        ...
        filters.regex("#requests")
    ) &
    filters.chat(chats=MYSUP)
)
async def request(bot, message):
    await message.reply_text(
            text="<i>Your request has been accepted! Please wait for some time, our Admins will upload it when they're online.</i>",
            reply_to_message_id=message.message_id
        )
    await bot.send_message(LOG_CHANNEL, script.REQUEST_TXT.format(message.from_user.id, message.from_user.mention, message.text))

@Client.on_message(filters.command("ping"))
async def ping(_, message):
    iFilms = await message.reply_sticker('CAACAgUAAxkBAAEFSk1i0kKUdhbaRNcbYkOgRTRzQLuaUgACYgUAArizkFYZM26MaqFq_ykE')
