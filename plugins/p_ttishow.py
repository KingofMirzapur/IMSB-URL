            ...
            for u in message.new_chat_members:
                if message.chat.id in filters.chat(chats=MYSUP):
                    try:
                       ...
                       [[
                          InlineKeyboardButton("ᴜᴘᴅᴀᴛᴇs", url=f"t.me/{UPDATES}")
                       ]])
                       k = await message.reply_photo(photo="https://telegra.ph/file/490f26ba76ecc9961c47c.jpg", caption=f"<b>ʜᴇʏ {u.mention} wᴇʟᴄᴏᴍᴇ ᴛᴏ {message.chat.title} !</b>\n\n<b>usᴇʀ ᴅᴇᴛᴀɪʟs 🥡 :</b>\n<b>{message.from_user.id}</b>\n\n<b>ʀᴇᴀᴅ ʀᴜʟᴇs ᴜsɪɴɢ</b> <code>/ʀᴜʟᴇ</code> <b>🎯</b>", reply_markup=button)
...
async def disable_chat(bot, message):
    ...
    try:
        ...
        await bot.send_message(LOG_CHANNEL, script.BANG_LOG_TXT.format(chat_, message.from_user.mention))
    except Exception as e:
        await message.reply(f"Error - {e}")
...
async def get_ststs(bot, message):
    ...
    reply_markup = InlineKeyboardMarkup(buttons)
    ifilmsbots = await message.reply('Fetching stats..')
...
async def list_users(bot, message):
    ifilmsbotz = await message.reply('Getting List Of Users')
    ...
    async for user in users:
        ...
        out += '\n'
    try:
        await ifilmsbotz.edit_text(out)
...
async def list_chats(bot, message):
    ifilmsbotz = await message.reply('Getting List Of chats')
